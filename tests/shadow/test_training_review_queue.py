...
def test_export_writes_triage_worksheets(tmp_path, capsys):
    src = tmp_path / "legacy.jsonl"
    src.write_bytes(
        b"\n".join(
            [
                _encode(_row()),
                _encode(_row(text="red sun sets", **{"class": "INFERRED", "license": "operator-attested"})),
                _encode(
                    _row(
                        text="green field waits",
                        **{"class": "OBSERVED", "license": "operator-attested; labels OBSERVED"},
                    )
                ),
            ]
        )
    ...
    assert triage["training_ready"] is False
    assert triage["by_priority_cohort"]
    assert (out / "worksheets").is_dir()
    assert list((out / "worksheets").glob("*.jsonl"))
    assert (out / "MANIFEST.json").is_file()
    for path in (out / "worksheets").glob("*.jsonl"):
        for line in path.read_text(encoding="utf-8").splitlines():
            row = json.loads(line)
            assert row["confirm_rights"] is False
            assert row["confirm_labels"] is False
            assert row["decision"] == "PENDING_REVIEW"


def test_validate_decisions_does_not_write_sidecar(tmp_path):
    row = _row()
    built = build_review_queue(_encode(row))
    item = copy.deepcopy(built["queue"][0])
    key = item["canonical_row_sha256"]
    # pending flags
    pending = validate_decisions(built["queue"], [{"canonical_row_sha256": key}])
    assert pending["report"]["n_pending"] == 1
    assert pending["report"]["sidecar_written"] is False
    assert pending["report"]["training_ready"] is False
    # operator-attested prefix rejected
    bad = copy.deepcopy(item)
    bad["confirm_rights"] = True
    bad["confirm_labels"] = True
    bad["draft"]["source"]["rights_status"] = "approved"
    bad["draft"]["source"]["rights_reference"] = "operator-attested; labels OBSERVED"
    bad["draft"]["annotation"]["labels"]["family"].update(
        status="reviewed", loss_mask=True, values=["ai-native"], reviewer="r1", method="human"
    )
    bad["draft"]["annotation"]["spans"] = []
    rejected = validate_decisions(built["queue"], [bad])
    assert rejected["report"]["n_rejected"] == 1
    assert "not authoritative" in rejected["rejected"][0]["reason"]
    # good synthetic ref would accept but still writes nothing
    good = copy.deepcopy(bad)
    good["draft"]["source"]["rights_reference"] = "fixture-rights-memo.md#1"
    good["draft"]["annotation"]["labels"]["structure"].update(
        status="unreviewed", loss_mask=False, values=[], reviewer=None, method="unreviewed"
    )
    ok = validate_decisions(built["queue"], [good])
    assert ok["report"]["n_would_accept"] == 1
    assert ok["report"]["sidecar_written"] is False
    assert list(tmp_path.iterdir()) == []


def test_cli_validate_operator_attested_prefix(tmp_path, capsys):
    src = tmp_path / "legacy.jsonl"
    src.write_bytes(_encode(_row()))
    out = tmp_path / "pkg"
    assert main(["build", "--input", str(src), "--out-dir", str(out)]) == 0
    capsys.readouterr()
    queue = json.loads((out / "queue.json").read_text(encoding="utf-8"))
    draft = copy.deepcopy(queue[0]["draft"])
    draft["source"]["rights_status"] = "approved"
    draft["source"]["rights_reference"] = "operator-attested"
    draft["annotation"]["labels"]["family"].update(
        status="reviewed", loss_mask=True, values=["ai-native"], reviewer="r", method="human"
    )
    draft["annotation"]["spans"] = []
    decisions = tmp_path / "decisions.jsonl"
    decisions.write_text(
    ...
    assert main(["validate", "--queue", str(out / "queue.json"), "--decisions", str(decisions)]) == 2
    result = json.loads(capsys.readouterr().out)
    assert result["report"]["n_rejected"] == 1
    assert result["report"]["sidecar_written"] is False
