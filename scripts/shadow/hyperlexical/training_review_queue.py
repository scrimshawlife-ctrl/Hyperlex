    ...
    (directory / "README.txt").write_text(
        "\n".join(
            [
                ...
                "",
                "1. Review queue.json. Draft fields are suggestions only.",
                "2. Confirm rights with an authoritative rights_reference (not operator-local/OBSERVED).",
                "3. Confirm per-head labels with reviewer ids; set loss_mask only for supervised heads.",
                "4. Write decisions JSONL with confirm_rights=true and confirm_labels=true.",
                "5. python -m scripts.shadow.hyperlexical.training_review_queue apply \\",
                "     --queue queue.json --decisions decisions.jsonl --out-metadata metadata.json",
                "6. python -m scripts.shadow.hyperlexical.training_intake --input original.jsonl \\",
                "     --metadata metadata.json --out-dir ../intake-NEW",
                "7. Then training_prepare + verify_preparation into a fresh directory.",
                "",
