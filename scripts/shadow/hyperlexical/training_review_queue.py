...
def materialize_sidecar_entry(item: dict) -> dict:
    ...
    if (
        cleaned in banned
        or cleaned.startswith("operator-local")
        or cleaned.startswith("operator-attested")
        or cleaned.startswith("operator-")
    ):
...
def validate_decisions(queue_items: list, decisions: list) -> dict:
    """Audit decisions without writing a sidecar. Never invents approvals."""
    by_key = {
        item["canonical_row_sha256"]: item
        for item in queue_items
        if isinstance(item, dict) and "canonical_row_sha256" in item
    ...
    for decision in decisions:
        ...
        key = decision.get("canonical_row_sha256")
        if key not in by_key:
            rejected.append({"canonical_row_sha256": key, "reason": "UNKNOWN_DIGEST"})
            continue
        if decision.get("confirm_rights") is not True or decision.get("confirm_labels") is not True:
            pending.append(
                {
                    "canonical_row_sha256": key,
                    "reason": "PENDING_CONFIRM_FLAGS",
                    "confirm_rights": decision.get("confirm_rights", False),
                    "confirm_labels": decision.get("confirm_labels", False),
                }
            )
            continue
        merged = json.loads(json.dumps(by_key[key]))
        if "draft" in decision:
            merged["draft"] = decision["draft"]
        merged["confirm_rights"] = True
        merged["confirm_labels"] = True
        try:
            materialize_sidecar_entry(merged)
        except (TypeError, ValueError) as exc:
            rejected.append({"canonical_row_sha256": key, "reason": str(exc)})
            continue
        accepted.append({"canonical_row_sha256": key, "reason": "WOULD_ACCEPT"})
    report = {
        "version": "training_review_validate.v1",
        "status": "VALIDATE_ONLY",
        "n_decisions": len(decisions),
        "n_would_accept": len(accepted),
        ...
        "note": (
            "Validation only. No sidecar written and no rights/labels approved. "
            "operator-* / OBSERVED rights_reference remain rejected."
        ),
    }
    return {
        "report": report,
...
def main(argv=None) -> int:
    ...
    validate = sub.add_parser("validate", help="Audit decisions without writing a sidecar")
    ...
    if args.cmd == "validate":
        ...
        for line in args.decisions.read_text(encoding="utf-8").splitlines():
            if line.strip():
                decisions.append(parse_json(line))
        result = validate_decisions(queue, decisions)
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result["report"]["n_rejected"] == 0 else 2
