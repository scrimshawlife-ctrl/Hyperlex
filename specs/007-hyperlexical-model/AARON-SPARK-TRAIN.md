...
This is a **seed smoke** for harness wiring, not a Hyperlexical card. A seed smoke is not an E2 result (trained E2 PASS lives on the climb BEST — see `STATUS.md`). Name-gate is false. Do not upload to Hugging Face. Do not say the model is Hyperlexical.
