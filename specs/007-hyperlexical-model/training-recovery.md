...
### Spark morph34 longer climb (2026-09-13)

- Continued as recommended after morph33: **6-epoch** guarded climb
  (`~/hlx/run_morph_train.sh 34`, guard `0.015`) into **new** out dir
  `~/.hyperlex/models/hyperlex-encoder-modernbert-base-seed-morph34`.
- Held morph19 envelope (no new lever): `slot_ce`, `HEAD_SLOT=2`, `OBSERVED=2`,
  `HARD=4`, `INFERRED=1.0`, `CAP=0`, curriculum `POS=1 TYPE=1`, `LAST=4`,
  `SECOND_SLOT` identity `1.0`, prior `hard_atoms_train.jsonl`.
- Val civilian (epoch 5 / joint): `unbind_exact≈0.383`, `unbind_token_f1≈0.660`,
  `classify_acc≈0.580` (n_unbind_eval=363). Below BEST morph19 `≈0.4545` →
  **REJECT** pin (decision recorded; BEST not retargeted).
- Epoch curve: 0.022 -> 0.0744 -> 0.2011 -> 0.2534 -> 0.3333 -> 0.3829.
  `25c523973fb5129b339dce333c90d717d3cc172ea45a9f76bfb22b8ad4cc451f` differs
  from morph19 `ec80ca179e008325cba2a7b73722b9c753212dcebe1175b1e181bcaa2685e7fd`
  (same live_included=4316; not a warm-start).
- Trunk-forward `eval_unbind` on morph34: probe `e2_pass=true` /
  `unbind_exact=1.0` (n=24). Probe E2 ≠ civilian ladder promotion.
- Residual themes: `partial_slot_miss=168`, `type_slot_token_miss=89`, `positional_head_filler_miss=42`, `full_miss=31`, `morph_bleed=10`.
- BEST/morph19 path + `model.safetensors` mtime unchanged
